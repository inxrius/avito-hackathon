...
export function TraitsPanel({ role, style }: Props) {
  ...
  return (
    <section className="panel recap-panel">
      ...
      {open && (
        <div className="why">
          {role.reason || style.reason ? (
            <>
              {role.reason && <p className="why__line">{role.reason}</p>}
              {style.reason && <p className="why__line">{style.reason}</p>}
            </>
          ) : (
            <p className="why__line">Обоснование недоступно для этих итогов.</p>
          )}
        </div>
      )}
    </section>

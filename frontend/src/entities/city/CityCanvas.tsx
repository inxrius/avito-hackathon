...
import { districtColor, shift } from '@/shared/lib/palette';
...
export function CityCanvas({
  ...
  const paint = useCallback(() => {
    ...
    for (const cell of painted) {
      ...
      drawGround(ctx, sx, sy, shift(districtColor(district), 0.3));
    }
    ...
    for (const cell of painted) {
      ...
      const { sx, sy } = project(cell.gx, cell.gy);
      const base = districtColor(district);
      const dimmed = highlight !== null && highlight !== cell.districtId;
  ...
  return (
    <div className="city">
      <canvas
        ...
        onClick={handleClick}
        role="img"
        aria-label={`«${recap.cityName}»: ${pluralize(recap.totals.districts, DISTRICTS)} за ${recap.year} год`}
      />
      ...
      {tooltip && (
        ...
        >
          <div className="city__tooltip-title">
            <span
              className="city__dot"
              style={{ background: districtColor(tooltip.district) }}
            />
            {tooltip.district.title}
          </div>
          <div className="city__tooltip-stats">
            {Math.round(tooltip.district.share * 100)}% года
            {tooltip.district.topCategory ? ` · улица «${tooltip.district.topCategory}»` : ''}
          </div>
        </div>
      )}
    </div>

...
export function buildCityLayout(
  ...
  // Этажность считаем от доли района, а не от счётчика действий:
  // счётчиков по районам бэкенд не отдаёт, а доля — отдаёт всегда.
  const maxShare = Math.max(...districts.map((d) => d.share), 0.0001);
  const cells: Cell[] = [];
  ...
  for (const district of ordered) {
    ...
    const intensity = district.share / maxShare;
